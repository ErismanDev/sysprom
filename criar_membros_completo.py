...
def criar_membros_completo():
    print("=== CRIANDO MEMBROS DE COMISSÕES COMPLETO ===\n")
    ...
    try:
        ...
        # Verificar se temos CargoFuncao padrão
        cargo_padrao, created = CargoFuncao.objects.get_or_create(
            nome="Membro de Comissão",
            defaults={
                'descricao': 'Membro padrão de comissão de promoções',
                'ativo': True,
                'ordem': 1
            }
        )
        ...
        for membro in membros_sqlite:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        # Gerar matrícula única baseada no CPF
                        matricula = f"MIG{cpf.replace('.', '').replace('-', '')[-6:]}"
                        ...
                        militar = Militar.objects.create(
                            ...
                            orgao_expedidor='SSP',
                            sexo='M',
                            email=f'{nome_guerra.lower()}@cbmepi.gov.br',
                ...
                if membro_existente:
                    ...
                    # Atualizar dados
                    membro_existente.tipo = tipo or 'EFETIVO'
                    membro_existente.cargo = cargo_padrao
                    membro_existente.ativo = bool(ativo)
                    if data_nomeacao:
                        membro_existente.data_nomeacao = datetime.strptime(data_nomeacao, '%Y-%m-%d').date()
                    else:
                        membro_existente.data_nomeacao = date.today()
                    if data_termino:
                        membro_existente.data_termino = datetime.strptime(data_termino, '%Y-%m-%d').date()
                    ...
                    if data_nomeacao:
                        novo_membro.data_nomeacao = datetime.strptime(data_nomeacao, '%Y-%m-%d').date()
                    else:
                        novo_membro.data_nomeacao = date.today()
                    
                    if data_termino:
                        novo_membro.data_termino = datetime.strptime(data_termino, '%Y-%m-%d').date()
