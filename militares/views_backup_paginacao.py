...
def quadro_fixacao_vagas_delete(request, pk):
    ...
    # Verificar se o usuário é membro de alguma comissão e tem permissão para excluir este quadro
    membro_comissao = MembroComissao.objects.filter(
        usuario=request.user,
        ativo=True,
        comissao__status='ATIVA'
...
def gerenciar_permissoes(request):
    """Interface principal para gerenciar permissões"""
...
def permissao_cargo_detail(request, cargo_id):
    """Detalhes das permissões de um cargo específico"""
...
def permissao_cargo_edit(request, cargo_id):
    """Editar permissões de um cargo"""
    cargo = get_object_or_404(CargoFuncao, pk=cargo_id)
    
    if request.method == 'POST':
        # Processar formulário de permissões
        permissoes_selecionadas = request.POST.getlist('permissoes')
        
        print(f"DEBUG: Recebidas {len(permissoes_selecionadas)} permissões")
        print(f"DEBUG: Permissões: {permissoes_selecionadas}")
        ...
        print(f"DEBUG: Removidas {permissoes_removidas} permissões existentes")
        ...
        for permissao_str in permissoes_selecionadas:
            if ':' in permissao_str:
                ...
                print(f"DEBUG: Criada permissão {modulo}:{acesso}")
        
        print(f"DEBUG: Total de permissões criadas: {len(permissoes_selecionadas)}")
        messages.success(request, f'Permissões do cargo "{cargo.nome}" atualizadas com sucesso!')
...
def perfil_acesso_create(request):
    ...
    if request.method == 'POST':
        ...
        if nome:
            ...
            messages.error(request, 'Nome do perfil é obrigatório!')
...
def usuario_permissoes_detail(request, usuario_id):
    """Detalhes das permissões de um usuário específico"""
    ...
    if request.method == 'POST':
        try:
            ...
            if cargos_com_perfil.exists():
                erro_protegido = True
                messages.error(request, f'Não é possível excluir este perfil porque {cargos_com_perfil.count()} cargo(s)/função(ões) estão usando este perfil.')
            elif permissoes_vinculadas.exists():
                erro_protegido = True
                messages.error(request, f'Não é possível excluir este perfil porque há {permissoes_vinculadas.count()} permissão(ões) vinculada(s).')
            else:
                perfil.delete()
                messages.success(request, 'Perfil de acesso excluído com sucesso!')
    ...
    if request.method == 'POST':
        try:
            ...
            if usuarios_com_funcao.exists():
                erro_protegido = True
                messages.error(request, f'Não é possível excluir este cargo/função porque {usuarios_com_funcao.count()} usuário(s) estão usando esta função.')
            elif permissoes_vinculadas.exists():
                erro_protegido = True
                messages.error(request, f'Não é possível excluir este cargo/função porque há {permissoes_vinculadas.count()} permissão(ões) vinculada(s).')
            else:
                cargo.delete()
                messages.success(request, 'Cargo/Função excluído com sucesso!')
                return redirect('militares:gerenciar_permissoes')
        except Exception as e:
            erro_protegido = True
    ...
    if request.method == 'POST':
        ...
        for perm_id in permissoes_selecionadas:
            ...
            if '_' in codename:
                ...
                modulo = modulo.upper()[:20]  # Truncar se necessário
                acesso = acesso.upper()[:20]  # Truncar se necessário
            else:
                modulo = 'OUTROS'
                acesso = 'VISUALIZAR'
...
def quadro_fixacao_vagas_visualizar_html(request, pk):
    ...
    for quadro_nome in itens_por_quadro:
        itens_por_quadro[quadro_nome].sort(key=lambda x: x.previsao_vaga.posto, reverse=True)
...
def almanaque_list(request):
    """Lista de almanaques armazenados"""
    ...
    context = {
        'title': 'Almanaques dos Militares',
    ...
    return render(request, 'militares/almanaque_list.html', context)
...
def almanaque_create(request):
    ...
    if request.method == 'POST':
        ...
        almanaque = AlmanaqueMilitar.objects.create(
            ...
            tipo=tipo,
            observacoes=observacoes,
            arquivo_pdf=ContentFile(pdf_content, name=f'almanaque_{tipo}_{timezone.now().strftime("%Y%m%d_%H%M%S")}.pdf')
        )
...
def gerar_pdf_almanaque(tipo):
    """Função auxiliar para gerar o conteúdo PDF do almanaque"""
    import os
    ...
    # Texto introdutório
    if tipo == 'OFICIAIS':
        texto_intro = f"Fica fixada a antiguidade dos oficiais do Corpo de Bombeiros Militar a partir do 18 de julho de 2025, conforme segue:"
    else:
        texto_intro = f"Documento gerado em {data_formatada} contendo a relação atualizada dos militares do Corpo de Bombeiros Militar do Estado do Piauí."
    story.append(Paragraph(texto_intro, style_just))
    story.append(Spacer(1, 16))
    ...
    if dados_organizados:
        for posto in ordem_postos:
            if posto in dados_organizados:
                militares = dados_organizados[posto]
                
                # Título do posto
                posto_display = dict(POSTO_GRADUACAO_CHOICES).get(posto, posto)
                story.append(Paragraph(f"{posto_display} ({len(militares)})", style_subtitle))
                story.append(Spacer(1, 10))
                
                # Tabela dos militares do posto
                header_data = [['Antiguidade', 'Nome Completo', 'Data da Promoção', 'Quadro']]
                
                for militar in militares:
                    # Formatar antiguidade
                    antiguidade = f"{militar.numeracao_antiguidade}º" if militar.numeracao_antiguidade else '-'
                    
                    # Formatar nome completo (primeira letra maiúscula)
                    nome_completo = militar.nome_completo.title() if militar.nome_completo else '-'
                    
                    # Formatar data de promoção
                ...
                if len(header_data) > 1:  # Se há militares além do cabeçalho
                    table = Table(header_data, colWidths=[2*cm, 8*cm, 3*cm, 3*cm])
                    table.setStyle(TableStyle([
                        ...
                        ('FONTNAME', (0, 1), (-1, -1), 'Helvetica'),
                        ('FONTSIZE', (0, 1), (-1, -1), 9),
                        ('ALIGN', (0, 1), (0, -1), 'CENTER'),  # Antiguidade centralizada
                        ('ALIGN', (1, 1), (1, -1), 'LEFT'),    # Nome alinhado à esquerda
                        ('ALIGN', (2, 1), (2, -1), 'CENTER'),  # Data centralizada
                        ('ALIGN', (3, 1), (3, -1), 'CENTER'),  # Quadro centralizado
    ...
    story.append(Paragraph(f"Documento gerado em: {timezone.now().strftime('%d/%m/%Y às %H:%M:%S')}", footer_style))
