#!/usr/bin/env python3
"""
Script para criar backup completo do sistema SEPROM CBMEPI - Versão 2
...
def backup_banco_dados(backup_dir, timestamp):
    ...
    try:
        # Tentar backup usando dumpdata com exceções específicas
        print("   🔄 Tentando backup com dumpdata...")
        
        # Backup específico de militares primeiro
        ...
        try:
            execute_from_command_line([
                ...
                '--exclude', 'militares.AlmanaqueAssinatura',
                '--exclude', 'militares.AlmanaqueAssinaturaCreate'
            ])
            print(f"   ✅ Militares salvos em: {militares_backup_file}")
        except Exception as e:
            print(f"   ⚠️ Erro no backup de militares: {str(e)}")
        
        # Backup de outros apps importantes
        apps_importantes = ['auth', 'contenttypes', 'sessions']
        for app in apps_importantes:
            try:
                app_backup_file = f"{backup_dir}/{app}_{timestamp}.json"
                execute_from_command_line([
                    'manage.py', 'dumpdata', 
                    app, 
                    '--indent=2', 
                    '--output', app_backup_file
                ])
                print(f"   ✅ {app} salvo em: {app_backup_file}")
            except Exception as e:
                print(f"   ⚠️ Erro no backup de {app}: {str(e)}")
...
def backup_scripts_documentacao(backup_dir):
    ...
    # Arquivos de script
    script_files = [f for f in os.listdir('.') if f.endswith('.py') and f != 'backup_completo_sistema_v2.py']
...
if __name__ == "__main__":
    print("🔧 Sistema de Backup Completo - SEPROM CBMEPI (v2)")
    print("=" * 50)
