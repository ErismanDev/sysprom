...
def migrar_membros_final():
    print("=== MIGRANDO MEMBROS DE COMISSÕES FINAL ===\n")
    ...
    try:
        ...
        # Buscar membros do SQLite com dados completos dos militares
        ...
        for membro in membros_sqlite:
            (id_sqlite, tipo, data_nomeacao, data_termino, ativo, observacoes,
             ...
             nome_completo, posto_graduacao, data_nascimento, data_incorporacao, 
             data_promocao_atual, data_ingresso, comissao_nome, comissao_tipo) = membro
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if data_nascimento:
                            militar_data['data_nascimento'] = datetime.strptime(data_nascimento, '%Y-%m-%d').date()
                        else:
                            militar_data['data_nascimento'] = date(1980, 1, 1)
                        
                        if data_ingresso:
                            militar_data['data_ingresso'] = datetime.strptime(data_ingresso, '%Y-%m-%d').date()
                        else:
                            militar_data['data_ingresso'] = date(2020, 1, 1)
                        
                        if data_promocao_atual:
                            militar_data['data_promocao_atual'] = datetime.strptime(data_promocao_atual, '%Y-%m-%d').date()
                        else:
                            militar_data['data_promocao_atual'] = date(2020, 1, 1)
