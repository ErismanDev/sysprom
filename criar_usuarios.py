#!/usr/bin/env python
"""
Script para criar usuarios do sistema SysProm - CBMEPI
Vincula usuarios aos militares cadastrados no sistema
"""
    ...
    for config in usuarios_config:
        try:
            ...
            if User.objects.filter(username=config['username']).exists():
                print(f"⚠ Usuario '{config['username']}' já existe")
                continue
            ...
            if config['militar_matricula']:
                try:
                    ...
                    print(f"✓ Usuario '{config['username']}' criado e vinculado ao militar {militar.nome_completo}")
                except Militar.DoesNotExist:
                    print(f"⚠ Usuario '{config['username']}' criado mas militar não encontrado")
            else:
                print(f"✓ Usuario '{config['username']}' criado (sem vinculo militar)")
                
        except Exception as e:
            print(f"❌ Erro ao criar usuario '{config['username']}': {e}")

def main():
    """Função principal"""
    print("=== CRIANDO USUARIOS DO SISTEMA SYSPROM - CBMEPI ===\n")
    
    try:
        with transaction.atomic():
            # 1. Criar grupos
            print("1. Criando grupos de usuarios...")
            grupos = criar_grupos()
            print()
            
            # 2. Criar cargos das comissoes
            print("2. Criando cargos das comissoes...")
            cargos = criar_cargos_comissao()
            print()
            
            # 3. Criar usuarios
            print("3. Criando usuarios...")
            ...
            print("- usuario/usuario123 (Usuario Geral)")
