...
# Script de Deploy Personalizado - SEPROM CBMEPI (Amazon Linux)
...
sudo yum install -y python3 python3-pip python3-devel nginx postgresql postgresql-server postgresql-contrib git curl gcc
...
sudo certbot --nginx -d sysprom.cbmepi.gov.br -d www.sysprom.cbmepi.gov.br --non-interactive --agree-tos --email admin@sysprom.cbmepi.gov.br || echo "SSL não configurado - configure manualmente"
...
echo "Para configurar SSL manualmente: sudo certbot --nginx -d sysprom.cbmepi.gov.br" 