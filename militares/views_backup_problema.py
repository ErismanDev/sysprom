...
from .models import (
    Militar, QuadroAcesso, ItemQuadroAcesso, 
    ...
    VagaManual, QuadroFixacaoVagas, ItemQuadroFixacaoVagas, AssinaturaQuadroFixacaoVagas,
    FichaConceitoOficiais, FichaConceitoPracas
)
from .forms import MilitarForm, FichaConceitoOficiaisForm, FichaConceitoPracasForm, DocumentoForm, UserRegistrationForm, ConfirmarSenhaForm, ComissaoPromocaoForm, MembroComissaoForm, SessaoComissaoForm, DeliberacaoComissaoForm, DocumentoSessaoForm, AtaSessaoForm, ModeloAtaForm, CargoComissaoForm
...
def militar_list(request):
    ...
    if ordenacao == 'hierarquia_antiguidade':
        ...
        # Anotar pontuação total considerando ambos os tipos de ficha
...
def militar_detail(request, pk):
    ...
    # Busca fichas de conceito (oficiais e praças)
    ficha_conceito_oficiais = militar.fichaconceitooficiais_set.all().order_by('-data_registro')
    ficha_conceito_pracas = militar.fichaconceitopracas_set.all().order_by('-data_registro')
    ficha_conceito = list(ficha_conceito_oficiais) + list(ficha_conceito_pracas)
...
def militar_dashboard(request):
    ...
    fichas_pendentes = (FichaConceitoOficiais.objects.count() + FichaConceitoPracas.objects.count())  # Removido filtro por status que não existe
    ...
    ultimas_fichas = FichaConceitoOficiais.objects.select_related('militar').order_by('-data_registro')[:5]
    ...
    # Calcular militares aptos para quadros de acesso
    aptos_antiguidade_oficiais = 0
    aptos_merecimento_oficiais = 0
    aptos_antiguidade_pracas = 0
    aptos_merecimento_pracas = 0
    
    # Debug: contar total de militares
    total_militares_ativos = Militar.objects.filter(situacao='AT').count()
    print(f"DEBUG: Total de militares ativos: {total_militares_ativos}")
    
    # Oficiais aptos por antiguidade (todos os oficiais ativos)
    oficiais_antiguidade = Militar.objects.filter(
        situacao='AT', 
        quadro__in=['COMB',  'SAUDE',  'ENG',  'COMP']
    )
    print(f"DEBUG: Total de oficiais ativos: {oficiais_antiguidade.count()}")
    
    for militar in oficiais_antiguidade:
        apto, motivo = militar.apto_quadro_acesso_simples()
        if apto:
            aptos_antiguidade_oficiais += 1
        else:
            print(f"DEBUG: Militar {militar.nome_completo} não apto: {motivo}")
    
    print(f"DEBUG: Oficiais aptos por antiguidade: {aptos_antiguidade_oficiais}")
    
    # Oficiais aptos por merecimento (apenas com ficha de conceito)
    oficiais_merecimento = Militar.objects.filter(
        situacao='AT', 
        quadro__in=['COMB',  'SAUDE',  'ENG',  'COMP'], 
        posto_graduacao__in=['CP',  'MJ',  'TC',  'CB']
    ).prefetch_related('fichaconceitooficiais_set', 'fichaconceitopracas_set')
    
    for militar in oficiais_merecimento:
        # Verificar se tem ficha de conceito
        if militar.fichaconceitooficiais_set.exists() or militar.fichaconceitopracas_set.exists():
            apto, _ = militar.apto_quadro_acesso_simples()
            if apto:
                aptos_merecimento_oficiais += 1
    
    print(f"DEBUG: Oficiais aptos por merecimento: {aptos_merecimento_oficiais}")
    
    # Praças aptas por antiguidade (todas as praças ativas)
    pracas_antiguidade = Militar.objects.filter(
        situacao='AT', 
        quadro='PRACAS'
    )
    print(f"DEBUG: Total de praças ativas: {pracas_antiguidade.count()}")
    
    for militar in pracas_antiguidade:
        apto, motivo = militar.apto_quadro_acesso_simples()
        if apto:
            aptos_antiguidade_pracas += 1
        else:
            print(f"DEBUG: Praça {militar.nome_completo} não apta: {motivo}")
    
    print(f"DEBUG: Praças aptas por antiguidade: {aptos_antiguidade_pracas}")
    
    # Praças aptas por merecimento (apenas com ficha de conceito)
    pracas_merecimento = Militar.objects.filter(
        situacao='AT', 
        quadro='PRACAS', 
        posto_graduacao__in=['2S',  '1S']
    ).prefetch_related('fichaconceitooficiais_set', 'fichaconceitopracas_set')
    
    for militar in pracas_merecimento:
        # Verificar se tem ficha de conceito
        if militar.fichaconceitooficiais_set.exists() or militar.fichaconceitopracas_set.exists():
            apto, _ = militar.apto_quadro_acesso_simples()
            if apto:
                aptos_merecimento_pracas += 1
    
    print(f"DEBUG: Praças aptas por merecimento: {aptos_merecimento_pracas}")
    
    # Teste simples: verificar se há militares no banco
    primeiro_militar = Militar.objects.filter(situacao='AT').first()
    if primeiro_militar:
        print(f"DEBUG: Primeiro militar: {primeiro_militar.nome_completo} - {primeiro_militar.posto_graduacao}")
        apto, motivo = primeiro_militar.apto_quadro_acesso_simples()
        print(f"DEBUG: Está apto? {apto} - Motivo: {motivo}")
    else:
        print("DEBUG: Nenhum militar ativo encontrado!")
    
    # Estatísticas por posto/graduação
    estatisticas_posto = {}
    
    # Para oficiais
    for posto in ['CB', 'TC', 'MJ', 'CP', '1T', '2T']:
        ...
        for militar in militares_posto:
            ...
            if apto:
                ...
                if militar.fichaconceitooficiais_set.exists() or militar.fichaconceitopracas_set.exists() and posto in ['CP', 'MJ', 'TC', 'CB']:
                    aptos_merecimento += 1
        
        estatisticas_posto[posto] = {
            'antiguidade': aptos_antiguidade,
            'merecimento': aptos_merecimento
        }
    
    # Para praças
    for graduacao in ['ST', '1S', '2S', '3S', 'CAB', 'SD']:
        militares_graduacao = Militar.objects.filter(
            situacao='AT', 
            posto_graduacao=graduacao, 
            quadro='PRACAS'
        ).prefetch_related('fichaconceitooficiais_set', 'fichaconceitopracas_set')
        
        aptos_antiguidade = 0
        aptos_merecimento = 0
        for militar in militares_graduacao:
            apto, _ = militar.apto_quadro_acesso_simples()
            if apto:
                aptos_antiguidade += 1
                # Para merecimento, verificar se tem ficha de conceito
                if militar.fichaconceitooficiais_set.exists() or militar.fichaconceitopracas_set.exists() and graduacao in ['2S', '1S']:
                    aptos_merecimento += 1
        
        estatisticas_posto[graduacao] = {
            'antiguidade': aptos_antiguidade,
            'merecimento': aptos_merecimento
        }
    ...
    context = {
        ...
        # Novos dados para aptos
        'aptos_antiguidade_oficiais': aptos_antiguidade_oficiais,
        'aptos_merecimento_oficiais': aptos_merecimento_oficiais,
        'aptos_antiguidade_pracas': aptos_antiguidade_pracas,
        'aptos_merecimento_pracas': aptos_merecimento_pracas,
...
from .forms import MilitarForm, DocumentoForm, UserRegistrationForm, ConfirmarSenhaForm, ComissaoPromocaoForm, MembroComissaoForm, SessaoComissaoForm, DeliberacaoComissaoForm, DocumentoSessaoForm, AtaSessaoForm, ModeloAtaForm, CargoComissaoForm
from django import forms
from django.contrib.auth import authenticate
from reportlab.pdfgen import canvas
from reportlab.lib.pagesizes import A4
...
def quadro_acesso_detail(request, pk):
    ...
    # Buscar militares disponíveis para o modal
    militares_no_quadro = quadro.itemquadroacesso_set.values_list('militar_id', flat=True)
    
    # Filtrar por posto específico se for merecimento
    if quadro.tipo == 'MERECIMENTO':
        # Para merecimento: apenas Capitão a Tenente-Coronel
        oficiais_elegiveis = Militar.objects.filter(
            situacao='AT', 
            posto_graduacao__in=['CP',  'MJ',  'TC']  # Capitão,  Major,  Tenente-Coronel
        ).exclude(
            id__in=militares_no_quadro
        ).order_by('posto_graduacao', 'nome_completo')
    else:
        # Para antiguidade: todos os oficiais
        oficiais_elegiveis = Militar.objects.filter(
            situacao='AT', 
            posto_graduacao__in=['CB',  'TC',  'MJ',  'CP',  '1T',  '2T',  'AS',  'AA']
        ).exclude(
    ...
    for oficial in oficiais_elegiveis:
        ...
        if apto:
            ficha = oficial.fichaconceitooficiais_set.first() or oficial.fichaconceitopracas_set.first()
            oficial.ficha_conceito = ficha
            oficial.pontuacao_ficha = float(ficha.pontos) if ficha and ficha.pontos else 0.0
            militares_disponiveis.append(oficial)
...
def militar_detail(request, pk):
    ...
    # Busca ficha de conceito (oficiais e praças)
    ficha_conceito_oficiais = militar.fichaconceitooficiais_set.all().order_by('-data_registro')
    ficha_conceito_pracas = militar.fichaconceitopracas_set.all().order_by('-data_registro')
    ficha_conceito = list(ficha_conceito_oficiais) + list(ficha_conceito_pracas)
...
def ficha_conceito_list(request):
    ...
    if militar_id:
        militar = get_object_or_404(Militar, pk=militar_id)
        fichas_oficiais = militar.fichaconceitooficiais_set.all().order_by('-data_registro')
        fichas_pracas = militar.fichaconceitopracas_set.all().order_by('-data_registro')
...
from .models import (
    ...
    POSTO_GRADUACAO_CHOICES, SITUACAO_CHOICES, QUADRO_CHOICES,
    PrevisaoVaga, AssinaturaQuadroAcesso, ComissaoPromocao, MembroComissao, SessaoComissao, PresencaSessao, DeliberacaoComissao, VotoDeliberacao, DocumentoSessao, AtaSessao, ModeloAta, CargoComissao,
    VagaManual, QuadroFixacaoVagas, ItemQuadroFixacaoVagas, AssinaturaQuadroFixacaoVagas  # <-- Adicionado
)
from .forms import MilitarForm, DocumentoForm, UserRegistrationForm, ConfirmarSenhaForm, ComissaoPromocaoForm, MembroComissaoForm, SessaoComissaoForm, DeliberacaoComissaoForm, DocumentoSessaoForm, AtaSessaoForm, ModeloAtaForm, CargoComissaoForm
from django import forms
from django.contrib.auth import authenticate
...
def quadro_acesso_pdf(request, pk):
    ...
    if assinaturas.exists():
        ...
        for i, assinatura in enumerate(assinaturas):
            ...
            from militares.utils import gerar_texto_assinatura_eletronica
            texto_assinatura = gerar_texto_assinatura_eletronica(
    ...
    if request.method == 'POST':
        ...
        if all([militar_id, posto_novo, criterio, data_promocao]):
            ...
            if not is_historica:
                ...
                # Atribuir próxima numeração de antiguidade disponível no novo posto
                from .utils import calcular_proxima_numeracao_antiguidade
                proxima_numeracao = calcular_proxima_numeracao_antiguidade(posto_novo, militar)
                
                militar.numeracao_antiguidade = proxima_numeracao
                militar.save()
                
                messages.success(request, f'Promoção registrada com sucesso! Militar {militar.nome_completo} recebeu a {proxima_numeracao}ª numeração de antiguidade no posto de {militar.get_posto_graduacao_display()}.')
            else:
...
def ficha_conceito_form(request, militar_pk):
    ...
    # Determinar se é oficial ou praça e buscar ficha existente
    if militar.is_oficial():
        ficha_existente = FichaConceitoOficiais.objects.filter(militar=militar).first()
        form_class = FichaConceitoOficiaisForm
        redirect_url = 'militares:ficha_conceito_list'
    else:
        ficha_existente = FichaConceitoPracas.objects.filter(militar=militar).first()
        form_class = FichaConceitoPracasForm
        redirect_url = 'militares:ficha_conceito_list'  # Sempre redirecionar para lista de oficiais
    
    if request.method == 'POST':
        if ficha_existente:
            # Se já existe, atualizar a ficha existente
            form = form_class(request.POST, request.FILES, instance=ficha_existente, militar=militar)
        else:
            # Se não existe, criar nova ficha
            form = form_class(request.POST, request.FILES, militar=militar)
        ...
        if ficha_existente:
            ...
            form = form_class(instance=ficha_existente, militar=militar)
        else:
            # Se não existe, criar formulário vazio
            form = form_class(militar=militar)
...
def ficha_conceito_detail(request, pk):
    """Visualiza detalhes de uma ficha de conceito"""
    # Primeiro, tentar buscar em FichaConceitoOficiais
    try:
        ficha = FichaConceitoOficiais.objects.get(pk=pk)
        tipo_ficha = 'oficiais'
    except FichaConceitoOficiais.DoesNotExist:
        # Se não encontrar em oficiais, tentar em praças
        try:
            ficha = FichaConceitoPracas.objects.get(pk=pk)
            tipo_ficha = 'pracas'
        except FichaConceitoPracas.DoesNotExist:
            raise Http404("Ficha de conceito não encontrada")
    
    militar = ficha.militar
    
    # Verificar se o tipo da ficha corresponde ao tipo do militar
    if militar.is_oficial() and tipo_ficha != 'oficiais':
        # Se o militar é oficial mas a ficha é de praças, buscar a ficha correta
        try:
            ficha = FichaConceitoOficiais.objects.get(militar=militar)
            tipo_ficha = 'oficiais'
        except FichaConceitoOficiais.DoesNotExist:
            pass  # Manter a ficha atual se não encontrar de oficiais
    elif not militar.is_oficial() and tipo_ficha != 'pracas':
        # Se o militar é praça mas a ficha é de oficiais, buscar a ficha correta
        try:
            ficha = FichaConceitoPracas.objects.get(militar=militar)
            tipo_ficha = 'pracas'
        except FichaConceitoPracas.DoesNotExist:
            pass  # Manter a ficha atual se não encontrar de praças
    ...
    if tipo_ficha == 'oficiais':
        pontos_detalhados = {
            ...
            'medalhas': {
                ...
                'pontos': min((ficha.medalha_federal * 0.5 + ficha.medalha_estadual * 0.3 +
                              ficha.medalha_cbmepi * 0.2), 1.0),
                'limite': 1.0,
...
def ficha_conceito_delete(request, pk):
    ...
    if request.method == 'POST':
        militar_nome = ficha.militar.nome_completo
        ficha.delete()
        messages.success(request, f'Ficha de conceito do militar {militar_nome} excluída com sucesso!')
        
        # Redirecionar para a lista correta baseada no tipo de ficha
        if tipo_ficha == 'pracas':
...
def documento_upload(request, ficha_pk):
    ...
    # Primeiro, tentar buscar em FichaConceitoOficiais
    try:
        ficha = FichaConceitoOficiais.objects.get(pk=ficha_pk)
    except FichaConceitoOficiais.DoesNotExist:
        # Se não encontrar em oficiais, tentar em praças
        try:
            ficha = FichaConceitoPracas.objects.get(pk=ficha_pk)
        except FichaConceitoPracas.DoesNotExist:
            raise Http404("Ficha de conceito não encontrada")
        ...
        try:
            ...
            if criterio_ordenacao in ['ANTIGUIDADE', 'MERECIMENTO']:
                try:
                    ...
                    print(f"DEBUG: Critério: {novo_quadro.tipo}")
        ...
        if categoria == 'OFICIAIS':
            if criterio == 'MERECIMENTO':
                candidatos = Militar.objects.filter(
                    Q(fichaconceitooficiais__isnull=False, 
                    situacao='AT', 
                    quadro__in=['COMB',  'SAUDE',  'ENG',  'COMP'], 
                    posto_graduacao__in=['CP',  'MJ',  'TC',  'CB']) | Q(fichaconceitopracas__isnull=False)
                )
            else:
                candidatos = Militar.objects.filter(
                    situacao='AT', 
            ...
            if criterio == 'MERECIMENTO':
                candidatos = Militar.objects.filter(
                    ...
                    situacao='AT', 
                    quadro='PRACAS', 
                    posto_graduacao__in=['2S',  '1S']) | Q(fichaconceitopracas__isnull=False)
                )
            else:
                candidatos = Militar.objects.filter(
                    situacao='AT', 
...
def gerar_fichas_conceito_todos(request):
    ...
    if request.method == 'POST':
        ...
        militares_sem_ficha = militares_ativos.filter(
            fichaconceitooficiais__isnull=True, 
            fichaconceitopracas__isnull=True
        )
...
def comissao_detail(request, pk):
    ...
    # Buscar quadros de acesso relacionados à comissão (por tipo)
    categoria_quadros = 'OFICIAIS' if comissao.tipo == 'CPO' else 'PRACAS'
    quadros_acesso = QuadroAcesso.objects.filter(
        categoria=categoria_quadros, 
        ativo=True
    ).order_by('-data_promocao')[:10]  # Últimos 10 quadros
    
    # Buscar quadros de fixação de vagas relacionados
    quadros_fixacao_vagas = QuadroFixacaoVagas.objects.filter(
        tipo=categoria_quadros, 
        status__in=['FINALIZADO',  'APROVADO']
    ).order_by('-data_promocao')[:10]  # Últimos 10 quadros
    
    # Comissões apenas visualizam quadros de fixação de vagas
    # Assinatura é competência exclusiva do Diretor de Gestão de Pessoas e/ou Chefe da Seção de Promoções
    pode_assinar_quadro_fixacao = False  # Comissões não assinam
    ...
    context = {
        ...
        'pode_assinar_quadro_fixacao': pode_assinar_quadro_fixacao,
    }
...
def comissao_encerrar(request, pk):
    """Encerrar comissão com data e opção de retornar"""
    from .forms import EncerrarComissaoForm
    ...
    if request.method == 'POST':
        form = EncerrarComissaoForm(request.POST)
        if form.is_valid():
            data_encerramento = form.cleaned_data['data_encerramento']
            observacoes = form.cleaned_data['observacoes']
            retornar_comissao = form.cleaned_data['retornar_comissao']
            
            if retornar_comissao:
                # Retornar comissão à ativa
                comissao.status = 'ATIVA'
                comissao.data_termino = None
                if observacoes:
                    comissao.observacoes = f"Reativada em {data_encerramento.strftime('%d/%m/%Y')}. {observacoes}"
                else:
                    comissao.observacoes = f"Reativada em {data_encerramento.strftime('%d/%m/%Y')}."
                comissao.save()
                messages.success(request, 'Comissão reativada com sucesso!')
            else:
                # Encerrar comissão
                comissao.status = 'INATIVA'
                comissao.data_termino = data_encerramento
                if observacoes:
                    comissao.observacoes = f"Encerrada em {data_encerramento.strftime('%d/%m/%Y')}. {observacoes}"
                else:
                    comissao.observacoes = f"Encerrada em {data_encerramento.strftime('%d/%m/%Y')}."
                comissao.save()
        ...
        form = EncerrarComissaoForm(initial=initial_data)
    ...
    return render(request, 'militares/comissao/encerrar.html', context)

@login_required
def exemplo_assinatura_eletronica(request):
    """Página de exemplo da assinatura eletrônica"""
    return render(request, 'militares/assinatura_eletronica_exemplo.html')
...
def ata_gerar_pdf(request, pk):
    ...
    if assinaturas_eletronicas.exists():
        for i, assinatura_eletronica in enumerate(assinaturas_eletronicas):
            ...
            from militares.utils import gerar_texto_assinatura_comissao
            texto_assinatura = gerar_texto_assinatura_comissao(
                assinatura_eletronica.membro,
                "Aprovação",
                assinatura_eletronica.data_assinatura
            )
...
def quadro_fixacao_vagas_pdf_view(request, pk):
    """Gera PDF do quadro de fixação de vagas para download"""
    from django.http import HttpResponse
    from reportlab.pdfgen import canvas
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.units import cm
    ...
    # Título centralizado e sublinhado
    titulo = f'<u>QUADRO DE FIXAÇÃO DE VAGAS - {quadro.get_tipo_display().upper()}</u>'
    story.append(Paragraph(titulo, style_title))
    story.append(Spacer(1, 16))

    # Texto do quadro de fixação de vagas para oficiais
    if quadro.tipo == 'OFICIAIS':
        texto_quadro = [
            "O DIRETOR DE GESTÃO DE PESSOAS DO CORPO DE BOMBEIROS MILITAR DO ESTADO",
            "DO PIAUÍ, no uso de suas atribuições que lhe confere o Art. 18, da lei 5.949, de 17 de dezembro de 2009,",
            "alterado pelo Art. 1° da lei 7.772, de 04 de abril de 2022;",
            "",
            "CONSIDERANDO o Anexo Único, da lei nº 5.949, de 17 de dezembro de 2009 (Lei de Organização",
            "Básica do CBMEPI), alterado pela Lei 7.772, de 04 de abril de 2022;",
            "",
            "CONSIDERANDO o Art. 2º da Lei nº 5.461 de 30 de junho de 2005 (Lei de Promoção de Oficiais",
            "do CBMEPI).",
            "",
            "CONSIDERANDO o Calendário de Promoção de Oficiais para o ano de 2025",            
            "",
            "RESOLVE:",
            "",
            "Art. 1º Fixar as vagas para as promoções de Oficiais em 18 de julho de 2025, conforme segue:"
        ]
        
        for linha in texto_quadro:
            if linha.strip():
                story.append(Paragraph(linha, style_just))
            else:
                story.append(Spacer(1, 6))
        
        story.append(Spacer(1, 12))
        
        # --- INSIRA O BLOCO DAS TABELAS AQUI ---
        for idx, (cod_quadro, grupo) in enumerate(grupos.items(), start=1):
            # Usar o nome completo do quadro
            ...
            nome_quadro_completo = quadro_nomes_completos.get(cod_quadro, f"QUADRO DE {grupo['nome'].upper()}")
            titulo_quadro = f"{idx} - {nome_quadro_completo}"
            story.append(Paragraph(titulo_quadro, style_bold))
            story.append(Spacer(1, 6))

            header = ['POSTO', 'PREVISTO', 'EXISTENTE', 'CLARO', 'VAGAS FIXADAS']
            data = [header]
            for item in grupo['itens']:
                posto_display = item.previsao_vaga.get_posto_display()
                previsto = item.previsao_vaga.efetivo_previsto
                existente = item.previsao_vaga.efetivo_atual
                claro = item.previsao_vaga.vagas_disponiveis
                vagas_fixadas = item.vagas_fixadas
                data.append([
                    posto_display,
                    previsto,
                    existente,
            ...
            table = Table(data, colWidths=[4*cm, 2.5*cm, 2.5*cm, 2.5*cm, 3*cm])
            table.setStyle(TableStyle([
    ...
    # Gerar código de verificação baseado no conteúdo do quadro
    texto_quadro = f"{quadro.numero} - {quadro.titulo} - {quadro.data_promocao}"
    codigo_verificacao = gerar_codigo_verificacao(texto_quadro)
    ...
    qr.add_data(f"Verificação: {codigo_verificacao}")
    qr.make(fit=True)
    ...
    codigo_texto = f"Código de Verificação: {codigo_verificacao}"
    story.append(Paragraph(codigo_texto, style_small))
    ...
    buffer.seek(0)
    response = HttpResponse(buffer, content_type='application/pdf')
    response['Content-Disposition'] = f'inline; filename="quadro_fixacao_vagas_{quadro.pk}_{quadro.get_tipo_display()}.pdf"'
...
def quadro_fixacao_vagas_create(request):
    ...
    if request.method == 'POST':
        ...
        if quadro_existente:
            messages.warning(request, f'Já existe um quadro de fixação de vagas para {quadro_existente.get_tipo_display()} na data {data_promocao.strftime("%d/%m/%Y")}.')
            return redirect('militares:quadro_fixacao_vagas_list')
        ...
        try:
            ...
            if data_automatica:
                messages.success(request, f'Quadro de Fixação de Vagas para {novo_quadro.get_tipo_display()} criado com sucesso! Data automática: {data_promocao.strftime("%d/%m/%Y")}')
            else:
                messages.success(request, f'Quadro de Fixação de Vagas para {novo_quadro.get_tipo_display()} criado com sucesso para {data_promocao.strftime("%d/%m/%Y")}!')
...
@login_required
def reordenar_antiguidade_promocao(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            total_reordenados = Militar.reordenar_numeracoes_por_antiguidade_promocao(
                posto_graduacao=posto_graduacao if posto_graduacao else None,
    ...
    return render(request, 'militares/reordenar_antiguidade_promocao.html', context)
...
def montar_quadro_acesso_oficiais(request, pk):
        ...
        if action == 'adicionar_militar':
            ...
            if not militar_id:
                ...
                try:
                    ...
                    if militar.posto_graduacao not in ['CB', 'TC', 'MJ', 'CP', '1T', '2T', 'AS', 'AA']:
                        ...
                        # Verificação simplificada de aptidão
                        apto = True
                        motivo = "Apto"
                        ...
                        if militar.situacao != 'AT':
                            apto = False
                            motivo = "Militar não está em situação ativa"
                        
                        # Verificar inspeção de saúde
                        elif not militar.apto_inspecao_saude:
                            apto = False
                            motivo = "Militar não está apto em inspeção de saúde"
                        
                        # Verificar validade da inspeção
                        elif militar.data_validade_inspecao_saude and militar.data_validade_inspecao_saude < timezone.now().date():
                            apto = False
                            motivo = "Inspeção de saúde vencida"
                        ...
                        if not apto:
                            ...
                            if quadro.tipo == 'MERECIMENTO':
                                # Para merecimento: não usar posição manual, pontuação vem da ficha
                                    ...
                                    messages.success(request, f'Oficial {militar.nome_completo} adicionado ao quadro com pontuação {pontuacao_decimal}!')
                            else:  # ANTIGUIDADE
            ...
            if militar_id:
                try:
                    ...
                    return redirect('militares:montar_quadro_acesso_oficiais', pk=quadro.pk)
            ...
            # Carregar automaticamente todos os oficiais aptos
            militares_no_quadro = quadro.itemquadroacesso_set.values_list('militar_id', flat=True)
            ...
            for oficial in oficiais_elegiveis:
                ...
                if apto:
                    ficha = oficial.fichaconceitooficiais_set.first() or oficial.fichaconceitopracas_set.first()
                    pontuacao_decimal = float(ficha.pontos) if ficha and ficha.pontos else 0.0
                    
                    if quadro.tipo == 'MERECIMENTO':
                        quadro.adicionar_militar_manual(oficial, None, pontuacao_decimal)
                    else:
                        quadro.adicionar_militar_manual(oficial, None, pontuacao_decimal)
                    
                    oficiais_adicionados += 1
            
            if oficiais_adicionados > 0:
                messages.success(request, f'{oficiais_adicionados} oficiais aptos foram adicionados automaticamente ao quadro!')
            else:
                messages.warning(request, 'Nenhum oficial apto encontrado para adicionar automaticamente.')
    ...
    for oficial in oficiais_elegiveis:
        ...
        if apto:
            ...
            oficial.ficha_conceito = ficha
            oficial.pontuacao_ficha = float(ficha.pontos) if ficha and ficha.pontos else 0.0
            oficial.ja_no_quadro = oficial.id in militares_no_quadro
    ...
    context = {
        ...
        'is_oficiais': True,
        'total_disponiveis': len(oficiais_disponiveis),
        'total_no_quadro': len(itens_quadro),
    }
...
def buscar_oficiais_elegiveis(request):
    ...
    if request.method == 'GET':
        ...
        # Se termo tem menos de 2 caracteres E não está vazio, retornar vazio
        ...
        try:
            ...
            # Se termo fornecido, filtrar por nome
            if termo:
                oficiais = base_qs.filter(nome_completo__icontains=termo)[:10]
            else:
                # Se termo vazio, retornar todos (limitado a 50 para performance)
                oficiais = base_qs[:50]
            ...
            for oficial in oficiais:
                ...
                if apto:
                    ...
                    militares_elegiveis.append({
                        ...
                        'pontuacao': float(ficha.pontos) if ficha and ficha.pontos else 0.0
                    })
