...
def gerar_quadro_acesso(request):
    ...
    if request.method == 'POST':
        ...
        # Verificar se já existe um quadro para este tipo e data
        ...
        if quadro_existente:
            messages.warning(request, f'Já existe um quadro de {quadro_existente.get_tipo_display().lower()} para a data {data_promocao.strftime("%d/%m/%Y")}.')
            return redirect('militares:quadro_acesso_list')
        ...
        try:
            novo_quadro = QuadroAcesso.objects.create(
                ...
                observacoes=f"Quadro de {tipo.lower()} para {data_promocao.strftime('%d/%m/%Y')} - Inclui todos os postos"
            )
...
@login_required
def ficha_conceito_list(request):
    """Lista ficha de conceito de um militar ou todas"""
    militar_id = request.GET.get('militar')
    if militar_id:
        militar = get_object_or_404(Militar, pk=militar_id)
        fichas = militar.fichaconceito_silitares = militares.filter(quadro=quadro)
    ...
    context = {
        'page_obj': page_obj,
        'militares': page_obj,
        'postos': t_posto_graduacao_display(),
        })
...
def ficha_conceito_list(request):
    ...
    if militar_id:
        ...
        militar = None
        fichas = FichaConceito.objects.all().order_by('-data_registro')
    ...
    militares_sem_ficha = total_militares_ativos - total_fichas
    
    context = {
        'militar': militar,
        'fichas': fichas,
        'total_militares_ativos': total_militares_ativos,
...
def quadro_acesso_list(request):
    ...
    # Adicionar quantidade de militares para cada quadro
    for quadro in quadros:
        quadro.total_militares_count = quadro.total_militartotal_militares': quadro.total_mil 'estatisticas': {
            'total': total_quadros,
            'elaborados': elaborados,
            'homologados': homologados,
            'nao_elaborados': nao_elaborados,
...
def militar_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        form = MilitarForm(instance=militart, 'militares/militar_confirm_delete.html', context)
...
def militar_search_ajax(request):
    ...
    militares = Militar.objects.filter(
        Q(nome_completo__icontains=query) |
 t()  # Removido filtro por status que não existe
    documentos_pendentes = Documento.objects.filter(status='PENDENTE').count()
    ...
    context = {
       ...
       .user.has_perm('militares.change_quadroacesso')
...
def militar_list(request):
    ...
    if ordenacao == 'hierarquia_antiguidade':
        ...
        militares = sorted(militares, key=lambda x: (
            ...
            x.numeracao_antiguidade or 999999,  # Militares sem antiguidade vão para o final
            x.nome_completo
     tar.nome_completo} atualizado com sucesso!')
            return redirect('militares:militar_detail', pk=militar.pk)
        else:
            messages.error(request, 'Erro ao atualizar militar. Verifique os dados.')
    else:
...
ficacoes_urgentes = notificacoes_base.filter(prioridade='URGENTE').count()
    notificacoes_altas = notificacoes_base.filter(prioridade='ALTA').count()
...
def quadro_acesso_list(request):
    ...
    elaborados = quadros.filter(status='ELABORADO').count()
    homologados = quadros.filter(status='HOMOLOGADO').count()
    nao_elaborados = quadros.filter(status='NAO_EL(request, 'militares/quadro_acesso_list.html', context)
...
def quadro_acesso_detail(request, pk):
    ...
    # Definir ordem dos quadros e transições (do mais titulo': '2º SARGENTO para o posto de 1º SARGENTO',
                    'origem': '2S',
                    'destino': '1S',
                    'texto': 'Deixa de ser elaborado o Quadro de Acesso por Antiguidade para o posto de 1º Sargento em virtude de não haver praça que satisfaça os requisitos essenciais para ingresso ao Quadro de acesso, nos termos do <b>art. 12</b> da Lei nº 5.461, de 30 de junho de 2005, alterada pela Lei Nº 7.772, de 04 de abril de 2022.'
                },
                {
                    ...
                    'origem': '3S',
                    'destino': '2S',
                    'texto': 'Deixa de ser elaborado o Quadro de Acesso por Antiguidade para o posto de 2º Sargento em virtude de não haver praça que satisfaça os requisitos essenciais para ingresso ao Quadro de acesso, nos termos do <b>art. 12</b> da Lei nº 5.461, de 30 de junhe abril de 2022.'
                },
                {
                    'numero': 'II',
                    'titulo': 'MAJOR para o posto de TENENTE-CORONEL',
...
def limpar_pontos_fichas_conceito(request):
    ...
    if request.method == 'POST':
        ...
        for ficha in fichas:
            ...
            FichaConceito.objects.filter(pk=ficha.pk).update(
...
def quadro_fixacao_vagas_create(request):
    ...
    if request.method == 'POST':
        ...
        if quadro_existente:
            messages.warning(request, f'Já existe um quadro de fixação de vagas para a data {data_promocao.strftime("%d/%m/%Y")}.')
...
def quadro_fixacao_vagas_oficiais_create(request):
    ...
    if request.method == 'POST':
        ...
        if quadro_existente:
            messages.warning(request, f'Já existe um quadro de fixação de vagas para oficiais na data {data_promocao.strftime("%d/%m/%Y")}.')
            return redirect('militares:quadro_fixacao_vagas_list')
