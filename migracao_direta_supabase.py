#!/usr/bin/env python3
"""
Script para migração direta sem associações automáticas
"""
...
def carregar_dados_direto(backup_file):
    """Carrega os dados diretamente sem associações automáticas"""
    print(f"\n📤 Carregando dados diretamente: {backup_file}")
    ...
    try:
        ...
        # Carregar dados usando loaddata com configurações especiais
        from django.core.management import call_command
        
        # Usar loaddata com configurações que evitam associações automáticas
...
def main():
    """Função principal"""
    print("🚀 MIGRAÇÃO DIRETA: Banco Local → Supabase")
    print("=" * 60)
    
    # Verificar se o arquivo de backup foi fornecido
    if len(sys.argv) != 2:
        print("❌ Uso: python migracao_direta_supabase.py <arquivo_backup>")
        print("Exemplo: python migracao_direta_supabase.py backup_atual_20250729_123645.json")
        return False
    ...
    # Passo 4: Carregar dados diretamente
    if not carregar_dados_direto(backup_file):
        return False
